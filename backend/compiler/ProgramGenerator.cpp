...
void ProgramGenerator::emitProgram(ADAParser::ProgramContext *ctx)
{
    ...
    emitInputScanner();
    emitConstructor();
    emitSubroutines(ctx->block()->declarations()->subprogramPart());
...
void ProgramGenerator::emitSubroutines(ADAParser::SubprogramPartContext *ctx)
{
    if (ctx != nullptr)
    {
        for (ADAParser::SubprogramBodyContext *defnCtx :
                                                    ctx->subprogramBody())
        {
...
void ProgramGenerator::emitMainMethod(ADAParser::ProgramContext *ctx)
{
    emitLine();
    emitComment("MAIN");
...
void ProgramGenerator::emitSubprogram(ADAParser::SubprogramBodyContext *ctx)
{
    ...
    // Emit code for the compound statement.
    ADAParser::CompoundStatementContext *stmtCtx =
        (ADAParser::CompoundStatementContext *) routineId->getExecutable();
