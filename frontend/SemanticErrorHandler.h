...
constexpr Error INVALID_PARAMETER 			= Error::INVALID_PARAMETER;
constexpr Error EXPECTED_RETURN 			= Error::EXPECTED_RETURN;
constexpr Error EXPECTED_EXIT 		     	= Error::EXPECTED_EXIT;
...
public:
    SemanticErrorHandler() : count(0), first(true)
    {
        ...
        SEMANTIC_ERROR_MESSAGES[INVALID_FIELD] =
                "Invalid field";
        SEMANTIC_ERROR_MESSAGES[INVALID_PARAMETER] =
                "IN/default parameter cannot be assigned";
        SEMANTIC_ERROR_MESSAGES[EXPECTED_RETURN] =
                "Expecting a return statement in functions";
        SEMANTIC_ERROR_MESSAGES[EXPECTED_EXIT] =
                "Expecting at lease one exit statement in a loop";
    }
